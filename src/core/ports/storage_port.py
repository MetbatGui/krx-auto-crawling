...
class StoragePort(ABC):
    ...
    def load_workbook(self, path: str) -> Optional[openpyxl.Workbook]:
        ...
        Args:
            path (str): 로드할 파일 경로 (상대 경로).
            
        Returns:
            Optional[openpyxl.Workbook]: 로드된 Workbook 객체. 파일이 없거나 로드 실패 시 None.
        """
        pass
    ...
    def ensure_directory(self, path: str) -> bool:
        ...
        Args:
            path (str): 디렉토리 경로 (상대 경로).
            
        Returns:
            bool: 생성 성공 여부 (이미 존재해도 True).
        """
        pass
    ...
    def load_dataframe(self, path: str, sheet_name: str = None, **kwargs) -> pd.DataFrame:
        ...
        Args:
            path (str): 파일 경로 (상대 경로).
            sheet_name (Optional[str]): 시트 이름. None이면 첫 번째 시트를 로드합니다.
            **kwargs: pandas.read_excel()에 전달할 추가 인자.

        Returns:
            pd.DataFrame: 로드된 DataFrame. 실패 시 빈 DataFrame 반환을 권장합니다.
        """
        pass
    ...
    def get_file(self, path: str) -> Optional[bytes]:
        ...
        Returns:
            Optional[bytes]: 파일 내용 (bytes). 파일이 없으면 None.
        """
        pass
