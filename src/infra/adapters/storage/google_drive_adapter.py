...
class GoogleDriveAdapter(StoragePort):
    ...
    def __init__(
        ...
        Args:
            ...
            client_secret_file: OAuth 2.0 Client Secret JSON 파일 경로
            root_folder_name: 데이터를 저장할 최상위 폴더 이름 (root_folder_id가 없을 때 사용)
            root_folder_id: 데이터를 저장할 최상위 폴더 ID (우선순위 높음)
        """
    ...
    def _get_file_id(self, path: str) -> Optional[str]:
        ...
        Args:
            path: 'folder/subfolder/file.ext' 형태의 경로
        """
