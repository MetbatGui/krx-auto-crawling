...
class LocalStorageAdapter(StoragePort):
    ...
    def __init__(self, base_path: str = "output", dry_run: bool = False):
        ...
        logger.info(f"[LocalStorage] 초기화 완료 (Base: {self.base_path.absolute()}, Dry-run: {self.dry_run})")
    ...
    def save_dataframe_excel(self, df: pd.DataFrame, path: str, **kwargs) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"[LocalStorage] [Dry-run] Would save Excel to: {path}")
            return True
        try:
            full_path = self.base_path / path
            self.ensure_directory(str(full_path.parent.relative_to(self.base_path)))
            df.to_excel(full_path, **kwargs)
            logger.info(f"[LocalStorage] Excel 저장 성공: {path}")
            return True
        except Exception as e:
            logger.error(f"[LocalStorage] Excel 저장 실패 ({path}): {e}")
            return False
    ...
    def save_dataframe_csv(self, df: pd.DataFrame, path: str, **kwargs) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"[LocalStorage] [Dry-run] Would save CSV to: {path}")
            return True
        try:
            full_path = self.base_path / path
            self.ensure_directory(str(full_path.parent.relative_to(self.base_path)))
            df.to_csv(full_path, **kwargs)
            logger.info(f"[LocalStorage] CSV 저장 성공: {path}")
            return True
        except Exception as e:
            logger.error(f"[LocalStorage] CSV 저장 실패 ({path}): {e}")
            return False
    ...
    def save_workbook(self, book: openpyxl.Workbook, path: str) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"[LocalStorage] [Dry-run] Would save Workbook to: {path}")
            return True
        try:
            full_path = self.base_path / path
            self.ensure_directory(str(full_path.parent.relative_to(self.base_path)))
            book.save(full_path)
            logger.info(f"[LocalStorage] Workbook 저장 성공: {path}")
            return True
        except Exception as e:
            logger.error(f"[LocalStorage] Workbook 저장 실패 ({path}): {e}")
            return False
    ...
    def load_workbook(self, path: str) -> Optional[openpyxl.Workbook]:
        ...
        try:
            ...
            logger.warning(f"[LocalStorage] 파일 없음: {path}")
            return None
        except Exception as e:
            logger.error(f"[LocalStorage] Workbook 로드 실패 ({path}): {e}")
            return None
    ...
    def ensure_directory(self, path: str) -> bool:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] 디렉토리 생성 실패 ({path}): {e}")
            return False
    ...
    def load_dataframe(self, path: str, sheet_name: str = None, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] DataFrame 로드 실패 ({path}): {e}")
            return pd.DataFrame()
    ...
    def get_file(self, path: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] 파일 읽기 실패 ({path}): {e}")
            return None
    def put_file(self, path: str, data: bytes) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"[LocalStorage] [Dry-run] Would save file (bytes: {len(data)}) to: {path}")
            return True
        try:
            full_path = self.base_path / path
            self.ensure_directory(str(full_path.parent.relative_to(self.base_path)))
            ...
            with open(full_path, 'wb') as f:
                f.write(data)
            logger.info(f"[LocalStorage] 파일 저장 성공: {path}")
            return True
        except Exception as e:
            logger.error(f"[LocalStorage] 파일 저장 실패 ({path}): {e}")
            return False
    def list_files(self, directory_path: str) -> list[str]:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] 파일 목록 조회 실패 ({directory_path}): {e}")
            return []
